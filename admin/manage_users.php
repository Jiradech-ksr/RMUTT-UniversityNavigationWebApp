...
// Query 1: Staff, Technicians, and Admins
...
// Query 2: Students only
$student_query = $conn->query("SELECT * FROM users WHERE role = 'student' ORDER BY created_at DESC");
?>
...
<ul class="nav nav-tabs" id="userTabs" role="tablist">
    ...
    <li class="nav-item" role="presentation">
        <button class="nav-link" id="student-tab" data-bs-toggle="tab" data-bs-target="#student-pane" type="button"
            role="tab">
            <i class="fas fa-user-graduate me-1"></i> นักศึกษา (Students)
            <span class="badge bg-success ms-1"><?= $student_query->num_rows; ?></span>
        </button>
    </li>
</ul>
...
<div class="tab-content shadow-sm" id="userTabContent">
    ...
    <div class="tab-pane fade show active" id="staff-pane" role="tabpanel" tabindex="0">
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    <?php if ($student_query->num_rows > 0):
                        while ($u = $student_query->fetch_assoc()): ?>
                            <tr class="<?= ($u['status'] == 'banned') ? 'table-secondary text-muted' : '' ?>">
                                <td class="px-4 py-2">
                                    <div class="d-flex align-items-center">
                                        <?php if (!empty($u['photo_url'])): ?>
                                ...
                                <td class="text-end px-4">
                                    ...
                                    <a href="?delete_id=<?= $u['id']; ?>" class="btn btn-sm btn-outline-danger"
                                        onclick="return confirm('ลบนักศึกษาคนนี้ออกจากระบบ?');"><i
                                            class="fas fa-trash-alt"></i></a>
                                </td>
                            </tr>
                        <?php endwhile; else: ?>
                        <tr>
                            <td colspan="4" class="text-center py-4 text-muted">ไม่มีข้อมูลนักศึกษา</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
